...
def run_experiment_csqa_local(dataset, model, tokenizer, device, num_examples, output_file, metadata):
    history = []

    example_rng = np.random.RandomState(1)
    num_problems = len(dataset)

    to_process = min(num_problems, 1000)
    for i in tqdm(range(to_process)):
        ...
        prompt_no_labels = utils.format_segments(dataset, [i], show_labels=metadata["show_labels"])
...
if __name__ == "__main__":
    ...
    num_examples = 32
    letters = "l" if metadata["show_labels"] else "nl"
    output_file = f"output/CSQA/csqa_train_qwen2.5-0.5b-instruct_k{num_examples}_{letters}.json"
